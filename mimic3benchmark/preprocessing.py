...
def assemble_episodic_data(stays, diagnoses):
    """
    combine info from `stays.csv` and `diagnoses.csv`
    """
    # ==| info from `stays` |==
    data = {'Icustay': stays.ICUSTAY_ID, 'Age': stays.AGE, 'Length of Stay': stays.LOS,
            'Mortality': stays.MORTALITY}
    data.update(transform_gender(stays.GENDER))               # encode gender to int
    data.update(transform_ethnicity(stays.ETHNICITY))         # encode race to int
    data['Height'] = np.nan
    data['Weight'] = np.nan
...
def extract_diagnosis_labels(diagnoses):
    """
    return a pivot table whose columns are only `diagnosis_labels`
    """
    ...
    for l in diagnosis_labels:
        if l not in labels:
            labels[l] = 0              
    labels = labels[diagnosis_labels]  # only wants labels in diagnosis_labels
...
def add_hcup_ccs_2015_groups(diagnoses, definitions):
    """
        in `hcup_ccs_2015.yaml` inload disease definitions and its `codes` 
        - `definition` : dict
        \t - `diease` : dict 
        \t \t - 'use_in_benchmark' : boolen
        \t \t - 'type' 
        \t \t - 'id' 
        \t \t - 'codes' : list
    """
    def_map = {}
    for dx in definitions:                              # definitions is a dic , dx as key
        for code in definitions[dx]['codes']:           # definitions[dx] is a dic, definitions[dx]['codes'] is a list
            def_map[code] = (dx, definitions[dx]['use_in_benchmark']) # for a code, def_map return dx name and whether used in benchmark
    # dx columns
...
def given_ITEMID(fn,txt_file):
    """
    for a set of ITEMID, extract variable info and treated as in benchmark 
    """
    # read the txt file
    item_ids=list(read_table(txt_file,sep=',',header=None).astype(str).values[0])
    while 'nan' in item_ids:
        item_ids.remove('nan')
    item_ids = DataFrame({'ITEMID':item_ids})
    # open the reference csv 
    var_map = dataframe_from_csv(fn, index_col=None).fillna('').astype(str)
    var_map.COUNT = var_map.COUNT.astype(int)
    
    #                          --------  SELECTION  ----------
    merged_df=var_map.merge(item_ids,how='inner',right_on=['ITEMID'],left_on=['ITEMID'])
    merged_df=merged_df[((merged_df.LEVEL2 != '') | (merged_df.ALTERNATIVE != ''))&(merged_df.COUNT.astype(int) > 10000)]
    #. fill into VARIABLE
    Is2 = (merged_df.LEVEL2 != '')
    merged_df.loc[Is2,'VARIABLE'] = merged_df.loc[Is2,'LEVEL2']
    IsAltNot2=(merged_df.LEVEL2 == '') & (merged_df.ALTERNATIVE != '')
    merged_df.loc[IsAltNot2,'VARIABLE'] = merged_df.loc[IsAltNot2,'ALTERNATIVE'] 
    #                          -------------------------------
    
    merged_df.ITEMID = merged_df.ITEMID.astype(int)
    merged_df = merged_df[['VARIABLE', 'ITEMID', 'MIMIC LABEL']].set_index('ITEMID')

    return merged_df.rename({'MIMIC LABEL': 'MIMIC_LABEL'}, axis=1), merged_df.VARIABLE.unique()


def STATUS_satisfying(var_map,STATUS):
    """
    return index of var_map satisfying condition STATUS (logical `or` for multiple conditions)
    """
    STATUS = [string.strip() for string in STATUS.split(',')]
    index = (var_map.STATUS == STATUS[0])
    for string in STATUS:
        index |=  (var_map.STATUS == string)
    return index

def read_itemid_to_variable_map(fn, STATUS='ready',variable_column='LEVEL2'):
    """
        from `itemid_to_variable_map.csv` select `ITEMID` whose: \n - LEVEL2 is not empty \n - STATUS is 'ready'
    """
    var_map = dataframe_from_csv(fn, index_col=None).fillna('').astype(str)
    # var_map[variable_column] = var_map[variable_column].apply(lambda s: s.lower())
    var_map.COUNT = var_map.COUNT.astype(int)
    #                          --------  SELECTION  ----------
    var_map = var_map[(var_map[variable_column] != '') & (var_map.COUNT > 0)]    # 372 ITEM left 
    if STATUS is not '': 
        var_map = var_map[STATUS_satisfying(var_map,STATUS)]                            # only 114 left !!!!
    #                          -------------------------------
...
def clean_events(events):
    global clean_fns              # a dict containing different function
    for var_name, clean_fn in clean_fns.items():
        idx = (events.VARIABLE == var_name)
        try:
            events.loc[idx, 'VALUE'] = clean_fn(events[idx])     # one function for one VARIABLE
