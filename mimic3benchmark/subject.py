...
def add_hours_elpased_to_events(events, dt, remove_charttime=True):
    """
    convert CHARTIME to HOURS since stays.INTIME
    """
    events = events.copy()
...
def convert_events_to_timeseries(events, variable_column='VARIABLE', variables=[]):
    '''
    map ITEMID in events.csv into variable and make it a pivot, remaining variable in `variables` will filled with nan
    '''
    ...
    for v in variables:
        if v not in timeseries:
            timeseries.loc[:,v] = np.full((timeseries.shape[0]),np.nan)       # a lot of nan is created here
    return timeseries
