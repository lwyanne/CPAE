...
def main():
    ...
    # return a iterator contain subdirectories that is subject folder
    ...
    for subject in tqdm(subjects, desc='Iterating over subjects'):
        ...
        # --------- open `stays.csv` from subject subdir --------------
        stays_df = pd.read_csv(os.path.join(args.subjects_root_path, subject, 'stays.csv'), index_col=False,
                               dtype={'HADM_ID': str, "ICUSTAY_ID": str})    # specific dtpye
        stays_df.columns = stays_df.columns.str.upper()                      # detail ??
        # -------------------------------------------------------------
        ...
        no_hadm_in_stay += (merged_df['_merge'] == 'left_only').sum()   #'left_only` points to null in events_df `HADM_ID`
        ...
        # here we assume that 1 `ICUSTAY_ID` match 1 `HADM_ID`
