    ...
    for patient in tqdm(patients, desc='Iterating over patients in {}'.format(partition)):
        ...
        for ts_filename in patient_ts_files:
            with open(os.path.join(patient_folder, ts_filename)) as tsfile:
                lb_filename = ts_filename.replace("_timeseries", "")       #  will open episode
                ...
                sample_times = np.arange(0.0, los + eps, sample_rate)    # time step of data

                sample_times = list(filter(lambda x: x > shortest_length, sample_times))  
                # patients should have at least record more than 4 hours
