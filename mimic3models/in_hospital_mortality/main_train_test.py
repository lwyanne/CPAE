...
import random
def draw_samples(data,seed,percentage):
    positives = list(data[data['y_true'] == 1].index.values.tolist())
    negatives = list(data[data['y_true'] == 0].index.values.tolist())
    ...
    data = data[['stay','y_true']].iloc[samples, :]
    return data
...
#                   ---------------------------
# percentages=[0.001,0.002,0.003,0.004,0.005,0.008,0.010,0.050,0.100,0.200,0.300,0.400,0.500,0.650,0.85,1.000]
percentages=[0.005]
accs=[]
roc_tests = []
prc_tests = []
...
for percentage in percentages:
    temp_list_file=draw_samples(train_list,seed,percentage)
...
# Build readers, discretizers, normalizers
    ...
    suffix = 'percentage_%f_seed_%d'%(percentage,seed)
    ...
    # Prepare training
    path = os.path.join(args.output_dir, 'keras_states/' + model.final_name + '.state')
    ...
    print("percentage %f==> training"%percentage)
...
#
    ...
    ap=results['ap']
    acc=results['acc']
    roc_tests.append(auroc)
    prc_tests.append(auprc)
    pses.append(minpse)
    accs.append(acc)
    aps.append(ap)
    # path = os.path.join(args.output_dir, "test_percentage%f_seed%d"%(percentage,seed) + ".csv")
