...
class Discretizer:
    ...
    def transform(self, X, header=None, end=None):
        ...
        N_bins = int(max_hours / self._timestep + 1.0 - eps)   # the same as `np.ceil` , project to bigger int


        #               ------|  fill in begin/end pos  |-----
        cur_len = 0
        ...
        for i in range(N_channels):
            ...
            cur_len = end_pos[i]  # dimension  of the whole input vector
                ...
                for channel in self._id_to_channel:
                    ...
                    if self._impute_strategy == 'previous':
                        if len(prev_values[channel_id]) == 0:
                            imputed_value = self._normal_values[channel]  # if no previous value, fill with normal value 
                        else:
