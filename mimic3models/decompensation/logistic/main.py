...
def main():
    ...
    train_reader = DecompensationReader(dataset_dir=os.path.join(args.data, 'train'),
                                        listfile=os.path.join(args.data, 'train_listfile_raw.csv'))
    ...
    imputer = Imputer(missing_values=np.nan, strategy='mean', axis=0, verbose=0, copy=True)  # Imputer of sklearn
    imputer.fit(train_X)
